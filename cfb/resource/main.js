...
function inflation(data, data2) {
    data.forEach((a, b, _) => {
        ...
        <tr class="no-drag">
            <th>${b + 1}</th>
            <td><a  href="/vote.php?word=${encodeURIComponent(a.term.split(',')[0])}&origin=${encodeURIComponent(a.origin)}" class="term">${a.term}</a></td>
            <td>${a.value}</td>
            <td>${a.percent}</td>
        </tr>
        `
    ...
    data2.forEach((a, b, _) => {
        ...
        <tr class="no-drag">
            <th>${b + 1}</th>
            <td><a  href="/output.php?nationality=korea&term=${encodeURIComponent(a.term.split(',')[0])}&origin=${encodeURIComponent(a.origin)}" class="term">${a.term}</a></td>
            <td>${a.value}</td>
            <td>${a.date}</td>
        </tr>
        `
